...
class SegNet(nn.Module):
    def __init__(self, input_channels, output_channels, VGG = False):
    ...
    def init_vgg_weigts(self):
        ...
        if self.input_channels == 1:
            nuova_features00 = torch.mean(self.vgg16.features[0].weight.data, dim = 1, keepdim=True)
            assert self.encoder_conv_00[0].weight.size() == nuova_features00.size()
            self.encoder_conv_00[0].weight.data = nuova_features00
            nuova_bias00 = self.vgg16.features[0].bias.data
            assert self.encoder_conv_00[0].bias.size() == nuova_bias00.size()
            self.encoder_conv_00[0].bias.data = nuova_bias00
        else:
