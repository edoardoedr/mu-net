...
if __name__ == "__main__":
    ...
    dataset_train = CT_dataset("/home/edofroses/Train_goldfish/dataset_train/", 3)
    dataset_train.convert(stack_type = "8bit")

    dataset_train.print_info()
    dataset_train.change_values([4, "="], 1)
    dataset_train.split_dataset()
    parametri = {"network" : "SEGNET", "tiles": 400, "batch_size":32, "numero_classi":4, "retrain":"", "num_epochs":100}
    ...
    dataset_test.inizializza_test("/home/edofroses/Train_goldfish/output_dataset_train_01/train_parameters.json")
    dataset_test.print_info()
    testing = deep_CT(dataset_test)
    testing.prediction(mode="2D", subprocess = True)
    dataset_test.change_values([4, "="], 1)
    dataset_test.performance()
    dataset_test.print_info()
    dataset_test.mode_filter_25D(filter_size = 3, tipologia = "prediction")
